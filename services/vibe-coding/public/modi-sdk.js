...
const SENSOR_SCHEMA = {
  ...
  [TYPE.ENVIRONMENT]: {
    intensity: 2, temperature: 2, humidity: 2, volume: 2,
    red: 3, green: 3, blue: 3, white: 3, black: 3, colorClass: 3, brightness: 3,
  },
  [TYPE.IMU]: {
    roll: 2, pitch: 2, yaw: 2, accX: 3, accY: 3, accZ: 3,
    angularVX: 4, angularVY: 4, angularVZ: 4, vibration: 5,
  },
};
...
const SENSOR_ACCESSORS = {
  ...
  [TYPE.BUTTON]: { clicked: "click", doubleClicked: "doubleClick", pressed: "pressedState", toggled: "toggle" },
  [TYPE.DIAL]: { turn: "turn", speed: "turnSpeed" },
  [TYPE.JOYSTICK]: { x: "coordinateX", y: "coordinateY", direction: "direction" },
  [TYPE.ENVIRONMENT]: {
    intensity: "intensity", temperature: "temperature", humidity: "humidity", volume: "volume",
    red: "red", green: "green", blue: "blue", white: "white", black: "black",
    colorClass: "colorClass", brightness: "brightness", brightnessPct: "intensity",
  },
  [TYPE.IMU]: {
    roll: "roll", pitch: "pitch", yaw: "yaw", accX: "accX", accY: "accY", accZ: "accZ",
    angularVX: "angularVX", angularVY: "angularVY", angularVZ: "angularVZ", vibration: "vibration",
  },
};
...
const WRITE_PROP = {
  LED_RGB: 16,
  SPEAKER_TUNE: 16, SPEAKER_MUSIC: 18, SPEAKER_MELODY: 19,
  MOTOR_SPEED: 17, MOTOR_ANGLE: 18, MOTOR_APPEND_ANGLE: 19, MOTOR_STOP: 20,
  DISPLAY_TEXT: 17, DISPLAY_PICTURE: 19, DISPLAY_RESET: 21,
  DISPLAY_VARIABLE: 22, DISPLAY_OFFSET: 25, DISPLAY_MOVE: 26,
};
...
function createEmitter() {
  ...
  return {
    ...
    emit(payload) { listeners.forEach((fn) => { try { fn(payload); } catch (e) { /* noop */ } }); },
...
class PostMessageTransport {
  ...
  start(handler) {
    ...
    if (typeof window !== "undefined") window.addEventListener("message", this._onWindowMessage);
  }
  stop() {
    if (typeof window !== "undefined") window.removeEventListener("message", this._onWindowMessage);
    this._handler = null;
  }
  send(message) {
...
class MockTransport {
  ...
  constructor({ modules, simulate, tickMs = 80, onActuator } = {}) {
    ...
    this.simulate = simulate || ((s) => {
      const t = s._t;
      s._set(TYPE.TOF, 1, SENSOR_SCHEMA[TYPE.TOF].distance, { distance: Math.round(15 + 14 * Math.sin(t / 12)) });
      s._set(TYPE.DIAL, 1, SENSOR_SCHEMA[TYPE.DIAL].turn, { turn: (t * 2) % 100, turnSpeed: 2 });
    });
  }
  ...
  start(handler) {
    ...
    this._timer = setInterval(() => {
      ...
      for (const { type, index, prop } of this._subs.values()) {
        ...
        if (value !== undefined) {
          this._handler({ type: MSG.GET_PROPERTY_RESPONSE, data: { moduleType: type, index, propertyNum: prop, value } });
        }
      }
    }, this.tickMs);
  }
  stop() { clearInterval(this._timer); this._timer = null; this._handler = null; }
  // 외부에서 버튼 클릭 등 이벤트 주입 (시연 UI 에서 사용)
  inject(type, index, prop, value) { this._set(type, index, prop, value); }
  send(message) {
    const { type, data } = message;
    if (type === MSG.GET_CONNECTED_MODI) {
      const connectModules = this.modules.map((m, i) => ({
        id: m.id ?? 10 + i, type: m.type, fieldIndex: m.index, index: m.index, uuid: `mock-${m.type}-${m.index}`,
      }));
      setTimeout(() => this._handler({ type: MSG.ON_CHANGE_CONNECT_MODI, data: { targetModule: null, connectModules } }), 0);
    } else if (type === MSG.GET_PROPERTY_REQUEST) {
      const key = `${data.moduleType}:${data.index}:${data.propertyNum}`;
      this._subs.set(key, { type: data.moduleType, index: data.index, prop: data.propertyNum });
...
class ModiBridge {
  ...
  onValueRaw(fn) { return this._raw.subscribe(fn); }

  /** 센서 변경 알림을 프레임당 1회로 묶는다 (하드웨어 푸시 속도와 리렌더 속도 분리 → 렉 방지). */
  _scheduleChange() {
    if (this._changeScheduled) return;
    this._changeScheduled = true;
    const fire = () => { this._changeScheduled = false; this._change.emit({ kind: "sensor" }); };
  ...
  useMock(opts) { return this.configure(new MockTransport(opts)); }
  usePostMessage(opts) { return this.configure(new PostMessageTransport(opts)); }
  ...
  _ensureTransport() {
    if (!this.transport) {
      ...
      this.configure(hasParent ? new PostMessageTransport() : new MockTransport());
    }
  }
  ...
  connect({ timeout = 4000 } = {}) {
    ...
    return new Promise((resolve) => {
      ...
      const off = this._modulesEmitter.subscribe(() => { off(); resolve(this.modules); });
      setTimeout(() => { off(); resolve(this.modules); }, timeout);
    });
  }
  ...
  _onMessage(msg) {
    if (msg.type === MSG.ON_CHANGE_CONNECT_MODI) {
      ...
      const d = msg.data || {};
      const t = d.moduleType, i = d.index;
      ...
      this._raw.emit({ moduleType: t, index: i, propertyNum: d.propertyNum, value: d.value });
      // 값이 실제로 바뀐 경우에만 리렌더 예약 → 정지 센서엔 리렌더 0, 변동도 프레임당 1회로 묶음
      if (prev === undefined || JSON.stringify(prev) !== JSON.stringify(d.value)) {
  ...
  _subscribe(type, index, prop, pollMs = POLL_MS) {
    ...
    const req = () => this.transport.send({ type: MSG.GET_PROPERTY_REQUEST, data: { moduleType: type, index: Number(index), propertyNum: prop, fast: true } });
    req();
    const timer = (typeof setInterval !== "undefined") ? setInterval(req, pollMs) : null;
  ...
  _write(type, index, propertyNum, value) {
    this._ensureTransport();
    this.transport.send({ type: MSG.SET_PROPERTY_REQUEST, data: { moduleType: type, index: Number(index), propertyNum, value } });
  }

  onChange(fn) { return this._change.subscribe(fn); }
  onModules(fn) { return this._modulesEmitter.subscribe(fn); }
  hasModule(type, index) { return this.modules.some((m) => m.type === type && (m.fieldIndex === index || m.index === index)); }
  reset() {
    for (const a of this._active.values()) a && a.timer && clearInterval(a.timer);
    this._active.clear(); this._cache = {};
  }
}
  ...
  return (index) => {
    ...
    for (const [name, key] of Object.entries(fields)) {
      ...
      Object.defineProperty(obj, name, {
        enumerable: true,
        get() { return bridge._read(type, index, prop)?.[key] ?? 0; },
      });
    }
...
function buildPublicApi(bridge) {
  const api = {
    _bridge: bridge,
    MSG, TYPE, SENSOR_SCHEMA, WRITE_PROP,
    ...
    get modules() { return bridge.modules; },
  ...
  /* 액추에이터 */
  api.led = (index) => ({
    setColor: (r, g, b) => bridge._write(TYPE.LED, index, WRITE_PROP.LED_RGB, { r, g, b }),
    off: () => bridge._write(TYPE.LED, index, WRITE_PROP.LED_RGB, { r: 0, g: 0, b: 0 }),
  });
  const motor = (type) => (index) => ({
    setSpeed: (speed) => bridge._write(type, index, WRITE_PROP.MOTOR_SPEED, { speed }),
    turnTo: (angle, speed = 70, mode = 0) => bridge._write(type, index, WRITE_PROP.MOTOR_ANGLE, { angle, speed, mode }),
    addAngle: (angle) => bridge._write(type, index, WRITE_PROP.MOTOR_APPEND_ANGLE, { angle, speed: 70 }),
  ...
  api.speaker = (index) => ({
    playTone: (frequency, volume = 50) => bridge._write(TYPE.SPEAKER, index, WRITE_PROP.SPEAKER_TUNE, { frequency, volume }),
    stop: () => bridge._write(TYPE.SPEAKER, index, WRITE_PROP.SPEAKER_TUNE, { frequency: 0, volume: 0 }),
  });
  api.display = (index) => ({
    text: (stringData) => bridge._write(TYPE.DISPLAY, index, WRITE_PROP.DISPLAY_TEXT, { stringData: String(stringData) }),
    clear: () => bridge._write(TYPE.DISPLAY, index, WRITE_PROP.DISPLAY_RESET, { option: 1 }),
  });
  ...
  api.onValue = (type, index, field, cb, opts = {}) => {
    ...
    return bridge.onValueRaw((d) => {
      if (d.moduleType === type && Number(d.index) === Number(index) && d.propertyNum === prop) {
...
function attachReactHooks(api, React) {
  ...
  api.useModi = () => {
    React.useSyncExternalStore(sub, () => bridge.modules.length, () => bridge.modules.length);
    return { modules: bridge.modules, connected: bridge.connected };
  };
  api.useTof = (i) => useSnapshot(() => api.tof(i).distance);
  api.useButton = (i) => {
    const b = api.button(i);
    useSnapshot(() => `${b.clicked}|${b.pressed}|${b.toggled}|${b.doubleClicked}`);
    return { clicked: b.clicked, pressed: b.pressed, toggled: b.toggled, doubleClicked: b.doubleClicked };
  };
  api.useDial = (i) => { const d = api.dial(i); useSnapshot(() => `${d.turn}|${d.speed}`); return { turn: d.turn, speed: d.speed }; };
  api.useJoystick = (i) => { const j = api.joystick(i); useSnapshot(() => `${j.x}|${j.y}|${j.direction}`); return { x: j.x, y: j.y, direction: j.direction }; };
...
if (typeof window !== "undefined") {
  ...
  if (window.React) {
    for (const h of ["useModi", "useTof", "useButton", "useDial", "useJoystick", "useEnv", "useImu", "useModiValue"]) {
