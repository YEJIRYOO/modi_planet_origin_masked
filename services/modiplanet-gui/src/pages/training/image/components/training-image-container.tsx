...
import ModelName from '@src/pages/training/image/components/ModelName';

interface ITrainingImageContainer {
  modelId: string;
  resetModelId: () => void;
}

function TrainingImageContainer({
  modelId,
  resetModelId,
}: ITrainingImageContainer) {
  const [modelUrl, setMyModelFromServer, resetMyModelImageClassifier] =
    useMyModelImageClassifier((state) => [
      state.modelUrl,
      state.setMyModelFromServer,
      state.reset,
    ]);
  const clearModel = useLearningModel((state) => state.clearModel);
