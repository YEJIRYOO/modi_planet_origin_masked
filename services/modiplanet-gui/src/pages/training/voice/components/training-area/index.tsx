...
import { TEST_AREA_ARROW_ENDPOINT_ID } from '@src/pages/training/voice/components/test-area';
import { ChevronDown } from '@src/lib/newAssets';
import useLearningVoiceModel, {
  TModelVariables,
} from '@src/pages/training/voice/hooks/useLearningVoiceModel';
import {
...
import TrainingOptionsContainer from '@src/pages/training/voice/components/training-area/training-options/training-options-container';
...
import TrainingStartButtonContainer from '@src/pages/training/voice/components/training-area/training-start-button/training-start-button-container';
import { TVoiceClassifier } from '@src/store/zustand/ai/my-model-voice-classifier';
...
import TrainingGuide from '@src/pages/training/voice/components/training-area/training-guide';
...
function TrainingArea() {
  ...
  const [model, setModel] = useVoiceModel((state) => [
    state.model,
    state.setModel,
  ]);
  ...
  const { modelTrain, modelLoad } = useLearningVoiceModel();
  ...
  const executeTraining = async () => {
    try {
      ...
      let data: Array<{ label: string; voiceUrl: string }> = [];

      classifiers.forEach(({ label, dataset }) => {
        dataset.forEach((voiceUrl) => {
          data.push({ label, voiceUrl });
        });
      });
